...
def aggregate_category(category: DbBoardCategory) -> None:
    ...
    category.last_posting_updated_at = (
        latest_posting.created_at if latest_posting else None
    )
    category.last_posting_updated_by_id = (
        latest_posting.creator_id if latest_posting else None
    )
...
def aggregate_topic(topic: DbTopic) -> None:
    ...
    latest_posting = posting_query.order_by(DbPosting.created_at.desc()).first()
