"""
byceps.services.shop.order.ordered_products_service
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def count_ordered_products(product_id: ProductID) -> dict[PaymentState, int]:
    """Count how often the product has been ordered, grouped by the
    ...
    db_line_items = (
        db.session.scalars(
            ...
            .options(
                ...
                db.joinedload(DbLineItem.product),
            )
        )
        .unique()
        .all()
...
def get_orders_including_product(
    product_id: ProductID, *, only_payment_state: PaymentState | None = None
) -> list[Order]:
    """Return all orders that contain the product.
